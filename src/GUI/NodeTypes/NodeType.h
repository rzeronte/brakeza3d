...
public:
    ...
    virtual void OnDelete(std::shared_ptr<Node>& node, ShaderNodeEditor* editor) {}

    // Configuración de pins
    virtual void SetupPins(std::shared_ptr<Node>& node, ShaderNodeEditor* editor) = 0;

    // Renderizado de UI personalizada
    virtual void RenderUI(std::shared_ptr<Node>& node, ShaderNodeEditor* editor) {}
