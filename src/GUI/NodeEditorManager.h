...
struct Pin {
    ...
    Pin(int _id, int _nodeId, ed::PinKind _kind, PinType _type, const std::string& _name)
...
struct Node {
    ...
    Node(int _id, const std::string& _name, ImVec4 _color = ImVec4(0.2f, 0.2f, 0.2f, 1.0f))
        : id(_id), name(_name), color(_color), size(ImVec2(0, 0)), userData(nullptr) {}
};
...
public:
    ...
    const std::vector<std::shared_ptr<Pin>>& GetPins() const { return m_Pins; }
    // ==================== Utilidades ====================
    void Clear();
