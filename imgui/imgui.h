...
namespace ImGui {
    ...
    IMGUI_API void
    NewFrame();                                 // start a new ImGui frame, you can submit any command from this point until Transforms()/EndFrame().
    IMGUI_API void
    EndFrame();                                 // ends the ImGui frame. automatically called by Transforms(), you likely don't need to call that yourself directly. If you don't need to render data (skipping rendering) you may call EndFrame() but you'll have wasted CPU already! If you don't need to render, better to not create any imgui windows and not call NewFrame() at all!
    IMGUI_API void
    ...
    GetDrawData();                              // valid after Transforms() and until the next call to NewFrame(). this is what you have to render. (Obsolete: this used to be passed to your io.RenderDrawListsFn() function.)
    ...
    GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Transforms(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO {
    ...
    // [OBSOLETE since 1.60+] Rendering function, will be automatically called in Transforms(). Please call your rendering function yourself now!
    // You can obtain the ImDrawData* by calling ImGui::GetDrawData() after Transforms(). See example applications if you are unsure of how to implement this.
    ...
    int MetricsRenderVertices;      // Vertices output during last call to Transforms()
    int MetricsRenderIndices;       // Indices output during last call to Transforms() = number of triangles * 3
...
struct ImDrawCmd {
    ...
    ImTextureID TextureId;              // User-provided texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed to Image*() functions. Ignore if never using levels or multiple fonts atlas.
...
struct ImDrawList {
    ...
    IMGUI_API void PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max,
                                bool intersect_with_current_clip_rect = false);  // Transforms-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData {
    bool Valid;                  // Only valid after Transforms() is called and before the next NewFrame() is called.
