...
public:
    ...
    CollisionResolverBetweenCamera3DAndTriggerMultiple(btPersistentManifold *contactManifold, Object3D *objA, Object3D *objB, BSPMap *bspMap, std::vector<Object3D*> *gameObjects, std::vector<Triangle *> &visibleTriangles) : CollisionResolver(contactManifold, objA, objB, bspMap, visibleTriangles)
    {
    ...
    void dispatch()
    {
        if (EngineSetup::getInstance()->LOG_COLLISION_OBJECTS) {
            Logging::getInstance()->Log("CollisionResolverBetweenCamera3DAndTriggerMultiple");
        }
        ...
        if (strlen(bspMap->getEntityValue(originalEntityIndex, "message")) > 0) {
            ComponentsManager::get()->getComponentHUD()->writeTextCenter(bspMap->getEntityValue(originalEntityIndex, "message"), false);
        }
