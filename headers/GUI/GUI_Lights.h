            ...
            for (int i = 0; i < lightPoints.size(); i++) {
                ...
                if (ImGui::CollapsingHeader(lightPoints[i]->label.c_str(), i)) {
                    ...
                    if (ImGui::TreeNode("Position")) {
                        ImGui::DragScalar("X", ImGuiDataType_Float, &lightPoints[i]->getPosition().x,
                                          range_position_sensibility, &range_position_min, &range_position_max, "%f",
                                          1.0f);
                        ImGui::DragScalar("Y", ImGuiDataType_Float, &lightPoints[i]->getPosition().y,
                                          range_position_sensibility, &range_position_min, &range_position_max, "%f",
                                          1.0f);
                        ImGui::DragScalar("Z", ImGuiDataType_Float, &lightPoints[i]->getPosition().z,
                                          range_position_sensibility, &range_position_min, &range_position_max, "%f",
                                          1.0f);
                        ImGui::TreePop();
                    }
                    ...
                    if (ImGui::TreeNode(attenuation_text.c_str())) {
                        ImGui::DragScalar("P", ImGuiDataType_Float, &lightPoints[i]->p, range_potence_sensibility,
                                          &range_potence_min, &range_potence_max, "%f", 1.0f);
                        ImGui::DragScalar("Constant", ImGuiDataType_Float, &lightPoints[i]->kc, range_sensibility,
                                          &range_min, &range_max, "%f", 1.0f);
                        ImGui::DragScalar("Linear", ImGuiDataType_Float, &lightPoints[i]->kl, range_sensibility,
                                          &range_min, &range_max, "%f", 1.0f);
                        ImGui::DragScalar("Quadratic", ImGuiDataType_Float, &lightPoints[i]->kq, range_sensibility,
                                          &range_min, &range_max, "%f", 1.0f);
                        ImGui::TreePop();
                    }

                    changed_color = ImGui::ColorEdit4(colorpicker_text.c_str(), (float *) &lightPoints[i]->imgui_color,
                                                      misc_flags);
