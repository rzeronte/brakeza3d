...
public:
    ...
    CollisionResolverBetweenCamera3DAndTriggerTeleport(btPersistentManifold *contactManifold, Object3D *objA,
    ...
    void dispatch() {
        ...
        int targetEntityId = bspMap->getIndexOfFirstEntityByTargetname(
                bspMap->getEntityValue(originalEntityId, "target"));
        ...
        if (this->bspMap->hasEntityAttribute(targetEntityId, "origin")) {
            ...
            ComponentsManager::get()->getComponentBSP()->getBSPCollider()->getPlayerModel()->teleportingCounter.setEnabled(
                    true);
            ComponentsManager::get()->getComponentBSP()->getBSPCollider()->resetPlayerModelData();

            Tools::playMixedSound(EngineBuffers::getInstance()->soundPackage->getSoundByLabel("teleporting"),
